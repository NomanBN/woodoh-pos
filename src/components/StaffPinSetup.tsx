  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'audit_logs'), {
        action: 'تعيين رمز الدخول الأول',
        performedBy: staff.name,
        performedByEmail: staff.email,
        targetTenantId: staff.tenantId,
        details: `قام الموظف ${staff.name} بتعيين رمز الدخول الخاص به لأول مرة`,
        timestamp: new Date().toISOString(),
        type: 'security'
      } as Omit<AuditLog, 'id'>);

      onSuccess({ ...staff, pin: hashedPin, mustChangePin: false });
    } catch (err) {
      console.error('Error setting PIN:', err);
      setError('حدث خطأ أثناء حفظ الرمز السري، يرجى المحاولة مرة أخرى');
    } finally {
      setLoading(false);
    }
  };
