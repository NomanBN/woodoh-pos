...
export default function Staff({ tenantId }: { tenantId: string }) {
  ...
  const [viewMode, setViewMode] = useState<'list' | 'performance' | 'permissions'>('list');
  ...
  const activeRoles = roles.reduce((acc, role) => {
    ...
    if (!existing) {
      acc.push(role);
    } else if (role.tenantId !== 'system' && existing.tenantId === 'system') {
  ...
  const staffWithPerformance = staff.map(member => {
    ...
    return {
      ...
      performance: {
        ...
        completed: memberOrders.filter(o => o.status === 'completed' || o.status === 'delivered').length,
        active: memberOrders.filter(o => o.status !== 'completed' && o.status !== 'delivered' && o.status !== 'cancelled').length
      }
    };
  });
  ...
  useEffect(() => {
    ...
    const qOrders = query(collection(db, 'orders'), where('tenantId', '==', tenantId));
    
    // Fetch both tenant roles and system roles in one query
    const rolesToFetch = tenantId ? ['system', tenantId] : ['system'];
    const qRoles = query(collection(db, 'roles'), where('tenantId', 'in', rolesToFetch));
    ...
    const unsubBranches = onSnapshot(query(collection(db, 'branches'), where('tenantId', '==', tenantId)), (snapshot) => {
      setBranches(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Branch)));
    });

    const unsubOverrides = onSnapshot(query(collection(db, 'user_permission_overrides'), where('tenantId', '==', tenantId)), (snapshot) => {
      const overridesData: Record<string, Partial<PermissionsMap>> = {};
      snapshot.docs.forEach(doc => {
        overridesData[doc.id] = doc.data().overrides;
      });
    ...
    try {
      ...
      if (editingStaff) {
        ...
        if (data.pin && data.pin.length === 4) {
          ...
          updateData.mustChangePin = isAutoGenerated;
        }
        ...
        await addDoc(collection(db, 'audit_logs'), {
          ...
          performedBy: currentStaff?.name || auth.currentUser?.displayName || 'المدير',
  ...
  const handleTogglePermission = async (roleId: string, key: PermissionKey) => {
    ...
    try {
      await updateRolePermissions(
        ...
        auth.currentUser?.email || 'admin@tailor.com',
        tenantId
      );
    } catch (err) {
      setToast({ message: 'فشل تحديث الصلاحية', type: 'error' });
      // Revert on error
      const originalRole = roles.find(r => r.id === roleId);
      if (originalRole) {
        setRoles(prev => prev.map(r => r.id === roleId ? originalRole : r));
      }
    }
  };
  ...
  const handleToggleStaffOverride = async (staffId: string, key: PermissionKey) => {
    ...
    // If no override exists, the effective value is the baseValue.
    // If an override exists, the effective value is the override value.
    ...
    // If the new value is the same as the base value, we can remove the override
    ...
    try {
      ...
      // Revert logic would be complex here, relying on onSnapshot to fix it
    }
  };
    ...
    <div className="p-6 space-y-8 text-right" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {canCreate && viewMode !== 'permissions' && (
            <button 
              onClick={() => {
                setEditingStaff(null);
                setIsModalOpen(true);
            ...
            <div className="flex items-center gap-2">
              {currentStaff?.email === "nomansa2566512@gmail.com" && roles.length === 0 && (
                <button 
                  ...
                  disabled={isSeeding}
                  className="bg-amber-500 text-white px-6 py-3 rounded-2xl flex items-center gap-2 hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/20 font-bold disabled:opacity-50"
                >
      ...
      {viewMode !== 'permissions' && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <select 
            ...
            className="bg-surface-muted border-2 border-transparent focus:border-brand rounded-2xl px-4 py-3 font-bold outline-none transition-all"
          >
          ...
          {filteredStaff.map((member) => (
            ...
            >
              <div className={`absolute top-0 right-0 w-2 h-full ${
                member.status === 'active' ? 'bg-emerald-500' : 'bg-surface-muted'
              }`} />
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                  <div className={`p-4 rounded-2xl transition-transform group-hover:scale-110 ${
                    ...
                    member.role === 'cashier' ? 'bg-blue-500/10 text-blue-600' : 'bg-orange-500/10 text-orange-600'
                  }`}>
                  ...
                  {canEdit && (
                    <div className="flex gap-1">
                      ...
                      <button 
                        ...
                        className="p-2 text-content-muted hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors"
              ...
              <div className="grid grid-cols-3 gap-2 pt-4 border-t border-border">
                ...
                <div className="text-center p-2 bg-emerald-50 rounded-2xl">
                  <p className="text-[10px] text-emerald-600 font-bold uppercase">منجز</p>
                  <p className="text-lg font-black text-emerald-600">{member.performance?.completed || 0}</p>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center pt-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs font-black ${
                    member.status === 'active' ? 'text-emerald-600' : 'text-content-muted'
                  }`}>
              ...
              {staffWithPerformance.map((member) => {
                ...
                return (
                  <tr key={member.id} className="hover:bg-surface-muted/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-black text-emerald-600">{member.performance?.completed}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            ...
                            role.roleKey === 'owner' ? "bg-amber-100 text-amber-600" : 
                            role.roleKey === 'manager' ? "bg-emerald-100 text-emerald-600" :
                    ...
                    <div className="bg-surface p-8 rounded-[2.5rem] border border-border shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                      <div className="flex items-center gap-4">
                        <div className={cn(
                          ...
                          selectedRoleForPermissions.roleKey === 'owner' ? "bg-amber-500 text-white shadow-amber-500/20" : 
                          selectedRoleForPermissions.roleKey === 'manager' ? "bg-emerald-500 text-white shadow-emerald-500/20" :
                                      ...
                                      <td className="p-6 text-center">
                                        <button
                                          ...
                                          className={cn(
                                            ...
                                            effectiveValue ? "bg-emerald-500" : "bg-surface-muted",
                                            isOwner && "opacity-50 cursor-not-allowed"
                                          )}
                                        >
                                          <div className={cn(
                                      ...
                                      <td className="p-6 text-center">
                                        <div className="flex flex-col items-center gap-1">
                                          <span className={cn(
                                            ...
                                            isOverridden ? "bg-indigo-100 text-indigo-600" : "bg-surface-muted text-content-muted"
                                          )}>
            ...
            {(showCreateRole || editingRole) && (
              <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
                ...
                <motion.div
                  ...
                  className="relative w-full max-w-md bg-surface rounded-[2.5rem] shadow-2xl border border-border p-8"
                >
        ...
        {selectedStaffForDetails && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-surface rounded-[2.5rem] shadow-2xl w-full max-w-2xl overflow-hidden relative z-10 max-h-[80vh] flex flex-col border border-border"
            >
              ...
              <div className="p-8 overflow-y-auto space-y-6">
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="bg-emerald-500/10 p-4 rounded-2xl text-center">
                    <p className="text-[10px] text-emerald-600 font-black uppercase mb-1">المنجزة</p>
                    <p className="text-2xl font-black text-emerald-600">{selectedStaffForDetails.performance?.completed}</p>
                  </div>
                </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-surface rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden relative z-10 border border-border"
            >
              <div className="p-6 border-b border-border flex justify-between items-center bg-surface-muted">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <select 
                    {...register('role')}
                    className="w-full bg-surface-muted border-2 border-transparent focus:border-brand rounded-2xl p-4 font-bold transition-all outline-none text-content"
                  >
                    {activeRoles.map(role => (
                      <option key={role.id} value={role.roleKey}>{role.name}</option>
                    ))}
      ...
      {/* Footer */}
      <div className="mt-12 py-8 border-t border-border flex flex-col items-center gap-4">
        <div className="flex items-center gap-2 text-content-muted">
          <span className="text-[10px] font-black uppercase tracking-widest">Powered By</span>
          <span className="text-xs font-black text-brand tracking-tighter">Wodoh Tech</span>
        </div>
        <p className="text-[8px] text-content-muted/50 font-bold uppercase tracking-[0.2em]">© 2026 Woodoh SaaS POS • All Rights Reserved</p>
      </div>
    </div>
  );
}
